...
public class Teamprojects1 {
	...
	public static void main(String[] args) {
		...
		while (run) {
			...
			String[][] tempArray = new String[100][5];	//정렬을 위한 배열

			System.arraycopy(boardArray, 0, tempArray, 0, tempArray.length);//배열 복사

			String[] temp;
			if (count <= 0) {//글의 개수가 없을경우
				System.out.println("글이 없습니다.");
			} else if(count < 100){
				for (int i = 0; i < 99; i++) {//정렬을 위한 코드
